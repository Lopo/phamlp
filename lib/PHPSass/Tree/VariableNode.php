...
extends Node
{
	...
	public function __construct($token)
	{
		...
		if (!empty($matches[VariableNode::SASS_ASSIGNMENT])) {
			$this->addWarning('Setting variables with "'.(!empty($matches[VariableNode::SASS_DEFAULT])? '||' : '').'=" is deprecated; use "$'.$this->name.': '.$this->value.(!empty($matches[VariableNode::SASS_DEFAULT])? ' !default' : ''));
			}
	}
