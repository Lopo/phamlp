...
/**
 * FunctionDefinitionNode class file.
...
extends Node
{
	...
	{
		// if ($token->level !== 0) {
		//   throw new FunctionDefinitionNodeException('Functions can only be defined at root level', $token);
		// }
		parent::__construct($token);
		preg_match(self::MATCH, $token->source, $matches);
		if (empty($matches)) {
			throw new FunctionDefinitionNodeException('Invalid Function', $token);
			}
