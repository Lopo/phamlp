...
/**
 * Colour class.
 * A Script object representing a CSS colour.
 *
...
extends Literal
{
	...
	public function __construct($colour)
	{
		if (is_string($colour)) {
			...
			if ($colour===self::TRANSPARENT) {
				...
				if (empty($matches)) {
					throw new ColourException('Invalid Colour string', ScriptParser::$context->node);
					}
		...
		elseif (is_array($colour)) {
			...
			elseif ($scheme=='hsl') {
				...
				$this->green=$colour[1];
				$this->blue=$colour[2];
				$this->alpha= isset($colour[3])
					? $colour[3]
					: 1;
				}
			}
		else {
			throw new ColourException('Colour must be a array', ScriptParser::$context->node);
			}
	}
	 ...
	 * @return mixed Colour the colour result
	 */
	 ...
	 * Colour subraction
	 *
	 * @param mixed $other value (Colour or Number) to subtract
	 ...
	 *
	 * @param mixed $other Colour|Number value to multiply by
	 * @return Colour the colour result
	 * @throws ColourException
	 */
	 ...
	 * Colour division
	 *
	 * @param mixed $other value (Colour or Number) to divide by
	 * @return Colour the colour result
	 * @throws ColourException
	 */
	 ...
	 * Colour bitwise AND
	 *
	 * @param mixed $other value (Colour or Number) to bitwise AND with
	 * @return Colour the colour result
	 * @throws ColourException
	 */
	 ...
	 * Colour bitwise OR
	 *
	 * @param mixed $other value (Colour or Number) to bitwise OR with
	 * @return Colour the colour result
	 * @throws ColourException
	 */
	 ...
	 * Colour bitwise XOR
	 *
	 * @param mixed $other value (Colour or Number) to bitwise XOR with
	 * @return Colour the colour result
	 * @throws ColourException
	 */
	 ...
	 *
	 * @return bool TRUE if this colour is translucent, FALSE if not
	 */
	 ...
	 *
	 * @param bool whether to use CSS3 SVG1.0 colour names
	...
	public function toString($css3=TRUE)
	{
		...
		if ($css3) {
			...
			return array_key_exists($colour, self::$_svgColours)
				? self::$_svgColours[$colour]
				: $colour;
			}
		return array_key_exists($colour, self::$_html4Colours)
			? self::$_html4Colours[$colour]
			: $colour;
	}
	...
	public function rgb2hsl()
	{
		...
		$s= $l>0.5
			? ((2-$max-$min)!=0? $d/(2-$max-$min) : 0)
			: (($max+$min)!=0? $d/($max+$min) : 0);
	 ...
	 * @param bool whether all colour space keys must be given
	...
	public function assertValid($colour, $all=TRUE)
	{
		if (array_key_exists('red', $colour) || array_key_exists('green', $colour) || array_key_exists('blue', $colour)) {
			if (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
				throw new ColourException('Colour can not have HSL and RGB keys specified', ScriptParser::$context->node);
				}
			if ($all&&(!array_key_exists('red', $colour) || !array_key_exists('green', $colour) || !array_key_exists('blue', $colour))) {
				throw new ColourException('Colour must have all RGB keys specified', ScriptParser::$context->node);
				}
		...
		if (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
			if ($all&&(!array_key_exists('hue', $colour) || !array_key_exists('saturation', $colour) || !array_key_exists('lightness', $colour))) {
				throw new ColourException('Colour must have all HSL keys specified', ScriptParser::$context->node);
				}

			return 'hsl';
			}
		if ($all && sizeof($colour)<3) {
			throw new ColourException('Colour array must have at least 3 elements', ScriptParser::$context->node);
			}
	}
